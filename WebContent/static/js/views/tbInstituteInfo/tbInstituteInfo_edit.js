...
function resetTbInstituteInfoForm() {
	$("#edit-TbInstituteInfo-form").form("reset");
	parent.addTabFun({
...
function editTbInstituteInfo() {

	// 开启验证框强制验证
	var bool = $("#edit-TbInstituteInfo-form").form('enableValidation').form('validate');
	if (bool) {
		// true就直接通过了
	} else {
		// 提示数据不完整
	...
	$.ajax({
		type : "POST",
		dataType : 'json',
		url : ctx + '/tbInstituteInfo/ajax_edit_tbInstituteInfo?oldName='
				+ oldName + '&oldOrder=' + oldOrder,
		data : $("#edit-TbInstituteInfo-form").serialize(),
