function addTbInstituteInfo() {
	// 开启验证框强制验证
			...
			var bool = $("#add-student-form").form('enableValidation').form('validate');
			if (bool) {
				//true就直接通过了
			} else {
				// 提示数据不完整
	...
	$.ajax({
		type : "POST",
		dataType : 'json',
		url : ctx + '/TbStudent/ajax_add_student',
		...
		success : function(data) {
			...
			if (data["code"] == '100') {
				...
				$.messager.alert('提示', data["msg"], data["code"]);
			}
		}
	});
}
											...
											.combobox(
													{
														...
														onSelect : function() {
															...
															//班级暂时搁置，已经生成的一套有问题，后台暂时写死
															var tiiId= $('#tsInstituteId').combobox('getValue');
															var tidiId= $('#tsInstituteDepartmentId').combobox('getValue');
															var tgId= $('#tsGrade').combobox('getValue');
															$('#tsClassId')
															.combobox(
																	{
																		url : ctx
																				+ '/TbClassInfo/ajax_getClass_byGradeId_AndInstituteId_AndInstituteDepartmentId?tc_institute_id='
